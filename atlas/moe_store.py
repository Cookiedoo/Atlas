...
def run_moe_iteration(store: ExperimentStore, root: str | Path, repository: str | Path | None = None, push: bool = False, seed: int = 0) -> dict[str, Any]:
    ...
    analysis = {"what_learned": "The router can mutate independently while all four expert blobs remain shared.", "what_changed": {"component": "router", "router_changed": mutation["changed"], "parent_score": parent_score, "child_score": child_score}, "what_did_not_change": {"expert_components": 4}, "runtime": {"device": str(reconstructed.device), "cuda_available": bool(torch.cuda.is_available()), "gpu": torch.cuda.get_device_name(0) if torch.cuda.is_available() else None}, "what_failed": None, "why": "A deterministic one-step router mutation was applied and reloaded, then scored against a fixed zero target.", "new_question": "Does router evolution improve a task benchmark?", "next_experiment": "Evaluate the reconstructed child on a task benchmark."}
    ...
    return {"experiment_id": experiment_id, "parent_candidate": parent_id, "child_candidate": child_id, "outcome": completed.outcome, "promoted": promoted, "score": child_score, "router_changed": mutation["changed"], "unchanged_expert_components": 4, "selected_experts": inference["selected_experts"], "device": str(reconstructed.device), "checkpoint": checkpoint}
